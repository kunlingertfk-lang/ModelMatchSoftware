...
void MatchResultItem::setResult(const MatchResult& res,bool flag) {
...
void MatchResultItem::setResult(const QVector<MatchResult>& resList,bool flag) {
    ...
    for(const auto& res : resList) {
        m_allPaths.append(convertXldToPath(res.modelContours));
    }
    update();
}
...
void MatchResultItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    ...
    if(!m_flag){
        ...
        painter->setRenderHint(QPainter::Antialiasing);//开启抗锯齿（让边缘更平滑）

        QPen xldPen(QColor(255,0,0),1);//红色
        ...
        for(int i = 0; i < m_resList.size(); ++i) {
            ...
            painter->drawLine(res.col - 10, res.row, res.col + 10, res.row);
            painter->drawLine(res.col, res.row - 10, res.col, res.row + 10);
            painter->drawText(res.col + 5, res.row - 5, QString::number(res.score, 'f', 2));
        }
    }
}
...
void MatchResultItem::clearContour(){
    m_score = 0.0f;
    m_label.clear();
    m_foundPos = {0.0,0.0};
    m_contourPath.clear();
